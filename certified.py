...
from torchvision.datasets.utils import _FILE_TYPE_ALIASES
from utils import *
...
def get_crown_loss(args, ub, lb, eps=None):
...
def get_bound_loss(args, model, loss_fusion, eps_scheduler, 
                    ...
                    meter=None, train=False, smooth_labels=None):
    if loss_fusion:
        c, bound_lower, bound_upper = None, False, True
    elif smooth_labels is not None:
        c, bound_lower, bound_upper = None, True, True
    else:
    ...
    if loss_fusion:
        ...
        if smooth_labels is not None:
            # CE loss with batch of smoothed labels
            # hard code worst-case logits without specification matrix

            worst_case_logits = lb.clone()
            incorrect_class_mask = (labels.unsqueeze(1) != torch.arange(args.num_class, device=args.device).unsqueeze(0))
            worst_case_logits = torch.where(
                incorrect_class_mask,
                ub,  # If incorrect class, use the upper bound (UB)
                worst_case_logits # If correct class, keep the lower bound (LB)
            )

            log_softmax = nn.LogSoftmax(dim=-1)(worst_case_logits)
            robust_loss = -torch.sum(smooth_labels * log_softmax, dim=-1).mean()
        else:
            # Pad zero at the beginning for each example, and use fake label '0' for all examples
            robust_loss = get_crown_loss(args, ub, lb, 
...
def cert(args, model, model_ori, epoch, epoch_progress, data, labels, eps, data_max, data_min, std, 
        ...
        train=False, meter=None, smooth_labels=None):
    ...
    if robust or reg or args.xiao_reg or args.vol_reg:
        b_res, robust_loss = get_bound_loss(args, model, loss_fusion, eps_scheduler, 
            ...
            eps=eps, meter=meter, train=train, smooth_labels=smooth_labels)
