...
# Patching np type error
def to_tensor_robust(pic):
    """Convert a PIL Image to a torch.Tensor robustly, normalizing to [0, 1]."""
    # Convert to NumPy array
    # Using np.float32 for consistency and to match PyTorch tensor type
    np_img = np.array(pic, np.float32, copy=False)

    if pic.mode == 'L': # Grayscale (MNIST)
        # Add channel dimension (1, H, W) and normalize 0-255 -> 0-1
        # FIX: Force a copy to create a standard NumPy array object before passing to torch.from_numpy.
        torch_tensor = torch.from_numpy(np.ascontiguousarray(np_img).copy()).unsqueeze(0) / 255.0
    elif pic.mode == 'RGB': # Color (CIFAR, TinyImageNet)
        # Change channel ordering HWC -> CHW (Transpose creates a non-contiguous array)
        np_img_transposed = np_img.transpose((2, 0, 1))
        # FIX: Force a copy to create a standard NumPy array object before passing to torch.from_numpy.
        torch_tensor = torch.from_numpy(np.ascontiguousarray(np_img_transposed).copy()) / 255.0
    else:
        # Fallback for unexpected modes, though should not happen for standard datasets
...
def load_data(args, data, batch_size, test_batch_size, use_index=False, aug=True):
    if data == 'MNIST':
        ...
        # patching type error with ToTensor
        train_data = MNIST('./data', train=True, download=True, transform=transforms.Lambda(to_tensor_robust), use_index=use_index)
        test_data = MNIST('./data', train=False, download=True, transform=transforms.Lambda(to_tensor_robust), use_index=use_index)
    ...
    # changed shuffle to False for indexing consistency for smooth labels
