...
def Train(model, model_ori, t, loader, eps_scheduler, opt, loss_fusion=False, valid=False, smooth=False):
    ...
    # smooth labels
    if smooth:
        embeddings, labels = extract_embeddings(model_ori, loader, args.device, args.num_class)
        # smooth_labels = smooth_labels_uniform(labels, args.device, num_classes=args.num_class, eps=eps_scheduler.get_eps())
        smooth_labels = smooth_labels_knn(embeddings, labels, args.device, k=args.k, sigma=args.sigma, num_classes=args.num_class)
    else:
        smooth_labels = None
    ...
    for i, (data, labels) in enumerate(loader):
        ...
        ### batch index
        if smooth and smooth_labels is not None:
            batch_size = data.size(0)
            batch_smooth_labels = smooth_labels[cur_smooth_idx : cur_smooth_idx + batch_size]
        else:
            batch_smooth_labels = None
        ...
        for k in range(grad_acc):
            ...
            regular_ce, robust_loss, regular_err, robust_err = cert(
                ...
                train=train, meter=meter, smooth_labels=batch_smooth_labels)
...
def main(args):
    ...
    if args.verify:
        ...
        for t in range(epoch + 1, args.num_epochs + 1):
            ...
            # start label smoothing at epoch smooth_start
            smooth = args.smooth and (t >= args.smooth_start)
            if lf:
                Train(model_loss, model_ori, t, train_data, eps_scheduler, opt, loss_fusion=True, smooth=smooth)
            else:
                Train(model, model_ori, t, train_data, eps_scheduler, opt, smooth=smooth)
